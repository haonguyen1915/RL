...
from rl.memory.memory_test import ReplayBuffer, PER_Memory
...
class DQNAgent(Agent):
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if len(self.memory) >= BATCH_SIZE:
            experiences = self.memory.sample(shuffle=True, type_data="tensor")
...
class DistPerspectiveDQNAgent(DQNAgent):
    ...
    def projection_distribution(self, next_state, rewards, dones):
        ...
        print(f"next_state: {next_state.size()}")  # [32, 4]
        print(f"rewards: {rewards.size()}")  # [32]
        print(f"dones: {dones.size()}")  # [32]
        print(f"next_action: {next_action.size()}")  # [32, 1, 51]
        print(f"next_dist: {next_dist.size()}")  # [32, 51]

        rewards = rewards.unsqueeze(1).expand_as(next_dist)
        dones = dones.unsqueeze(1).expand_as(next_dist)
        support = support.unsqueeze(0).expand_as(next_dist)
        print(f"rewards: {rewards.size()}")  # [32, 51]
        print(f"dones: {dones.size()}")  # [32, 51]
        print(f"support: {support.size()}")  # [32, 51]
        ...
        print(f"proj_dist: {proj_dist.size()}")  # [32, 51]
        exit()
